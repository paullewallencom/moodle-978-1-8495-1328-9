...
#var_dump($state->options, $numericalquestion);
        ...
        foreach ($dataset as $key => $multiply) {
            ...
            $name = preg_replace('#_?\d$#', '', $key); #rtrim($name, 's');
            $class= rtrim($name, 's');

            if (!isset(self::$default_pix[$class])) { #in_array($class, self::$wildcards)) {
                $class = 'unknown';
                $item = " <i>?</i>";
            } else {
            ...
            #$items = str_repeat("<li>$name</li>", $multiply);
            #$objects[] = "<ul class='$class' title='$class'>$items</ul>\n";
        }
        // Create the objects/pictures string.
        $object_str = $objects[0] ."<p class='op'>$op_replace</p>". $objects[1];
        #$object_str= str_replace($op, "<div class='op'>$op_replace</div>", $qtext);
        #$object_str= str_replace(array_keys($dataset), $objects, $object_str);
        #$object_str= str_replace(array('{', 's}', '}'), '', $object_str);

        $plain_str = str_replace(array_keys($dataset), $plains, $qtext);
        #$plain_str = preg_replace('#-?\d}#', '', $plain_str);
        $plain_str = str_replace(array('{', '}'), '', $plain_str);
        $plain_str = preg_replace("#\[[\+\-\*\/]\]#", '', $plain_str);

        $numericalquestion->questiontext = <<<EOT
        <h3 class="qco-text" style="text-align:center">$plain_str</h3>
        <div class="qco-objects">$object_str
